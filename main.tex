...
\title[Isoperimetric Comparison: Ricci Flow on Three Manifolds]
 {Isoperimetric Comparisons For Ricci Flow on Three Manifolds with Positive Curvature}
...
&= I^2I'' + I(I')^2 + I'\einsteinhilbert({\Omega_0}) - 4\pi\chi(\bdry{\Omega_0})
\end{split}
\end{equation}
...
\[
\partial_t \bar{\varphi} \leq \bar{\varphi}^2 \bar{\varphi}'' - \bar{\varphi}(\bar{\varphi}')^2 + \abs{M_t}^{-1/n} \einsteinhilbert(M_t)\left(\frac{2}{n} \bar{\varphi} - y \bar{\varphi}'\right) + \abs{M_t}^{-1/n} \einsteinhilbert(\Omega_0) \bar{\varphi}' - 4\pi\chi(\bdry{\Omega_0}).
\]
and define
\[
...
\mu &\mapsto c^{n/2} \mu \\
\Sc &\mapsto c^{-1} \Sc \\
\einsteinhilbert &\mapsto c^{(n-2)/2} \einsteinhilbert.
\end{align*}
In particular
\[
\abs{M_t}^{-1/n} \einsteinhilbert \mapsto c^{(n-3)/2} \abs{M_t}^{-1/n} \einsteinhilbert
\]
is not scale invariant but does scale exactly as the other terms in the differential inequality from the Comparison Thereom \ref{thm:comparison} (see the proof below). In particular, for \(n=2\), \(\einsteinhilbert\) is already scale invariant and equal to \(8\pi\chi(M)\) by Gauss-Bonnet and the differential inequality scales like \(\abs{M_t}^{-1/2}\). The case \(n=3\) is particularly special in that it is in fact scale invariant!
\end{rem}
...
\begin{split}
\partial_t \varphi - \varphi^2 \varphi'' - \varphi(\varphi')^2 &= \abs{M_t}^{(n-3)/n}\left[ \partial_{\tau} \bar{\varphi} -\bar{\varphi}^2 \bar{\varphi}'' - \bar{\varphi}(\bar{\varphi}')^2\right] \\
&\quad + \left(\frac{n-1}{n} \bar{\varphi} - y\bar{\varphi}'\right) \abs{M_t}^{-1/n} \einsteinhilbert(M_t).
\end{split}
\end{equation}
Note in particular, by the remark after the theorem statement, the entire equation \eqref{eq:scaled_diff_ineq} scales like \(\abs{M_t}^{(n-3)/n}\).

The proof now follows by the maximum principle, since \(I\) is a viscosity sub-solution whilst \(\varphi\) is a viscosity super-solution by the hypotheses of the theorem.
\end{proof}
